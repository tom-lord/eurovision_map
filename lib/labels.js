...
{
    return jQuery.inArray(code.toUpperCase(), CONFIG.contestants_entered()) != -1
}

function country_voted_only(code)
{
    return jQuery.inArray(code.toUpperCase(), CONFIG.contestants_voted_only()) != -1
}

function country_score(to_country)
{
    var score = 0;
    for(var from_country in CONFIG.from_country_rankings()) //TODO: One day, this will be configurable
    {
        if( from_country != to_country.toUpperCase())
        {
            score += (VOTING.from_1975_to_present[ CONFIG.from_country_rankings()[from_country][to_country] - 1 ] || 0);
        }
    }
    return score;
}
...
function country_position(code)
{
    ...
    jQuery.each(CONFIG.contestants_entered(), function(idx) 
    {
...
{
    return CONFIG.contestants_entered().length;
}
